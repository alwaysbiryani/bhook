...
function Row({
  ...
  return (
    <div className={`flex items-center justify-between py-1.5 text-sm ${strong ? "font-semibold" : ""}`}>
      <span className={muted ? "text-steel-dim" : "text-steel"}>{label}</span>
      <span className={muted ? "text-steel-dim" : "text-chalk"}>{children}</span>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-steel/10 bg-ink-2/60 p-4">
      ...
      <div className="mb-3 flex items-center justify-between rounded-xl bg-turmeric/12 px-3.5 py-2.5">
        <span className="text-sm font-medium text-turmeric">You saved</span>
        <PriceTicker value={bill.saved} className="text-xl font-bold text-turmeric" />
      </div>
      ...
      {showCouponButton && (
        ...
        >
          ...
          <span className="flex-1 text-sm font-medium text-turmeric">
      ...
      <div className="border-t border-steel/10 pt-2">
        <Row label="Item total">
          ...
          {bill.mrpSaving > 0 && (
            <span className="tnum ml-2 text-xs text-steel-dim line-through">{rupee(bill.itemMrp)}</span>
          )}
        </Row>
        ...
        {bill.coupons.map((c) => (
          <div key={c.code} className="flex items-center justify-between py-1.5 text-sm">
            ...
            <span className="tnum text-turmeric">
        ...
        <Row label="Delivery fee">
          <span className="tnum mr-2 text-xs text-steel-dim line-through">{rupee(bill.deliveryOrig)}</span>
          <span className="font-semibold text-veg">FREE</span>
        </Row>
        <Row label="Platform fee">
          <span className="tnum mr-2 text-xs text-steel-dim line-through">{rupee(bill.platformOrig)}</span>
          <span className="font-semibold text-veg">WAIVED</span>
        </Row>
        <Row label={<>GST <span className="text-xs text-steel-dim">(imaginary)</span></>} muted>
          <span className="tnum">{rupee(bill.gst)}</span>
        </Row>
        <Row label={<>Dabba covers the rest <span className="text-xs text-steel-dim">(kyunki aana nahi hai)</span></>}>
          <span className="tnum text-turmeric">−{rupee(bill.treat)}</span>
        </Row>
      </div>

      <div className="mt-2 flex items-center justify-between border-t border-steel/15 pt-3">
        <span className="font-display text-lg text-chalk">To pay</span>
