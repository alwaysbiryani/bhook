  ...
  return (
    <>
      ...
      <main className="mx-auto w-full max-w-md flex-1 px-5 py-6">
        <h1 className="font-display text-3xl text-chalk">Checkout</h1>
        ...
        {/* Order line */}
        <div className="mt-4 flex items-center justify-between rounded-xl border border-steel/10 bg-ink-2/40 px-4 py-3">
          <div>
            <p className="text-sm font-medium text-chalk">{restaurant.name}</p>
            <p className="text-xs text-steel-dim">{bill.count} items · {restaurant.area}</p>
          </div>
          <Link href="/cart" className="text-sm font-semibold text-bandhani">Edit</Link>
        </div>

        {/* Payment method — our own UPI-app visual language */}
        <h2 className="mt-6 mb-2 text-sm font-bold uppercase tracking-wider text-steel-dim">Pay using</h2>
            ...
            return (
              <button
                ...
                className={`flex w-full items-center gap-3 rounded-xl border px-4 py-3 text-left transition ${
                  on ? "border-bandhani bg-bandhani/10" : "border-steel/12 hover:border-steel/25"
                }`}
              >
                <span className={`grid h-5 w-5 place-items-center rounded-full border-2 ${on ? "border-bandhani" : "border-steel/30"}`}>
                  {on && <span className="h-2.5 w-2.5 rounded-full bg-bandhani" />}
                </span>
                <span className="flex-1">
                  <span className="block text-sm font-medium text-chalk">{m.label}</span>
                  <span className="block text-xs text-steel-dim">{m.sub}</span>
                </span>
              </button>
            );
          })}
        ...
        {/* method detail */}
        <div className="mt-3 rounded-xl border border-steel/10 bg-ink-2/40 p-4">
          {method === "upi" && (
            <div>
              <p className="text-xs text-steel-dim">Paying from</p>
              <p className="tnum mt-1 text-chalk">bhookha@dabba</p>
              <div className="mt-3 flex flex-wrap gap-2">
                {["Dabba Pay", "Any UPI", "Scan QR"].map((x) => (
                  <span key={x} className="rounded-lg border border-steel/15 px-3 py-1.5 text-xs text-steel">{x}</span>
                ))}
              </div>
            </div>
          )}
          {method === "card" && (
            <div>
              <p className="tnum text-chalk">•••• •••• •••• 0000</p>
              <p className="mt-1 text-xs text-steel-dim">A card we invented. We never asked for yours, and never will.</p>
            </div>
          )}
          {method === "cod" && (
            <p className="text-sm text-steel">
              You&rsquo;ll pay <span className="tnum text-chalk">₹0</span> in cash to a rider who isn&rsquo;t coming, for food that isn&rsquo;t real. Deal?
            </p>
          )}
        </div>
        ...
        <div className="mt-5 flex items-center justify-between rounded-xl bg-turmeric/12 px-4 py-3">
          <span className="text-sm font-medium text-turmeric">You&rsquo;re saving</span>
          <PriceTicker value={bill.saved} className="text-lg font-bold text-turmeric" />
        </div>

        <button
          type="button"
          onClick={startPay}
          className="mt-4 mb-4 flex w-full items-center justify-center gap-2 rounded-xl bg-bandhani px-5 py-4 text-lg font-semibold text-chalk shadow-pop transition active:scale-[0.99]"
        >
