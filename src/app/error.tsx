...
export default function Error({ reset }: { error: Error; reset: () => void }) {
  return (
    <main className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center px-6 py-24 text-center">
      ...
      <h1 className="mt-4 font-display text-2xl text-fg">Something didn&rsquo;t come through.</h1>
      ...
      <div className="mt-8 flex gap-3">
        <button
          onClick={reset}
          className="rounded-xl bg-bandhani px-6 py-3 font-medium text-fg shadow-pop transition active:scale-95"
        >
          Try again
        </button>
        <Link href="/" className="rounded-xl border border-line/20 px-6 py-3 font-medium text-fg">
          Back home
        </Link>
      </div>
    </main>
