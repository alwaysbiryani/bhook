    ...
    <>
      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-5 py-6">
        <nav className="mb-3 flex flex-wrap items-center gap-1.5 text-xs text-dim">
          <Link href={`/${c.slug}`} className="hover:text-fg">{c.name}</Link>
          <span>/</span>
          <Link href={`/${c.slug}/${r.slug}`} className="hover:text-fg">{r.name}</Link>
          <span>/</span>
        ...
        <div className="mt-4 flex items-center gap-2">
          <VegMark diet={d.diet} />
          {d.bestseller && (
            <span className="rounded bg-turmeric/15 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-turmeric-text">★ Bestseller</span>
          )}
        </div>
        <h1 className="mt-1.5 font-display text-4xl leading-tight text-fg">
          {d.name}
          {d.nameDeva && <span className="font-deva ml-3 text-2xl text-dim">{d.nameDeva}</span>}
        </h1>
        ...
        <div className="mt-3 flex items-center gap-2">
          <span className="tnum text-2xl font-semibold text-fg">{rupee(d.basePrice)}</span>
          <span className="tnum text-dim line-through">{rupee(d.mrp)}</span>
          {off > 0 && <span className="text-sm font-semibold text-turmeric-text">{off}% off</span>}
        </div>

        <p className="mt-4 text-lg leading-relaxed text-muted">{d.description}</p>

        <div className="mt-4 flex flex-wrap gap-x-5 gap-y-1 text-sm text-dim">
        ...
        <div className="mt-6">
          <AddButton
            dishSlug={d.slug}
            label="Add to cart"
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-bandhani px-5 py-4 text-lg font-semibold text-fg shadow-pop transition active:scale-[0.99]"
          />
        </div>

        {/* Recipe stub — SEO + the joke */}
        <section className="mt-10">
          <h2 className="font-display text-2xl text-fg">How it&rsquo;s (not) made</h2>
          <ol className="mt-3 space-y-3">
            {RECIPE.map((step, i) => (
              <li key={i} className="flex gap-3 text-muted">
                <span className="tnum grid h-6 w-6 shrink-0 place-items-center rounded-full bg-card text-sm text-turmeric-text">{i + 1}</span>
                <span>{step}</span>
              </li>
            ))}
          </ol>
        ...
        <div className="mt-8 rounded-2xl border border-line/10 bg-card/50 p-5 text-center">
          <p className="text-muted">More from <span className="text-fg">{r.name}</span>?</p>
          <Link href={`/${c.slug}/${r.slug}`} className="mt-3 inline-block rounded-xl border border-line/20 px-5 py-2.5 text-sm font-semibold text-fg hover:border-line/40">
            See the full menu
          </Link>
        </div>
      </main>
